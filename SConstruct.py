...
def buildSim(cppFlags, dir, type, pgo=None):
    ...
    pinInclDir2 = joinpath("/work2/z50038971/dynamorio/clients/drpin/drpin/include/")
    if not os.path.exists(joinpath(pinInclDir2, "pin.H")):
        pinInclDir2 = joinpath(pinInclDir2, "pin")
        assert os.path.exists(joinpath(pinInclDir2, "pin.H"))

    # Pin 2.14 changes location of XED
    xedName = "xed2"  # used below
    xedPath = joinpath(PINPATH, "extras/" + xedName + "-intel64/include")
    xedPath2 = joinpath("/work2/z50038971/dynamorio/clients/drpin/drpin/include/pin/include")
    if not os.path.exists(xedPath2):
        xedName = "xed"
      #  xedPath = joinpath(PINPATH, "extras/" + xedName + "-intel64/include")
        assert os.path.exists(xedPath2)

    env["CPPPATH"] = [xedPath2,
            pinInclDir2, joinpath(pinInclDir, "gen"),
    ...
    env["PINLINKFLAGS"] = " -Wl,--hash-style=sysv -Wl,-Bsymbolic -Wl,--version-script=" + joinpath(pinInclDir2, "pintool.ver")
    ...
    env["PINLIBPATH"] = ["/work2/z50038971/dynamorio/clients/drpin/drpin/build"]
